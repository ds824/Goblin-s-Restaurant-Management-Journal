...
public class Customer : MonoBehaviour
{
    ...
    IEnumerator EatAndLeave()
    {
        ...
        // [수정됨] 레시피 매니저를 통해 현재 가격을 가져오는 대신 레시피 자체 가격 사용 (구조에 따라 다름)
        ...
        if (UnityEngine.Random.Range(0f, 100f) < finalTipChance)
        {
            ...
            Debug.Log($"[팁 발생!] (최종매력: {finalCharmStat}, 특성: {traitTipBonus}%) 보너스로 {tip}G 팁 획득! (확률: {finalTipChance:F1}%)");
        }
    ...
    void CalculateSatisfaction()
    {
        ...
        // 1등급이 가장 높은 등급(대개)이므로 점수를 가장 많이 줌
